...
export async function DELETE(request: Request) {
  try {
    ...
    const { count } = await supabaseAdmin
      .from('income_transactions').select('id', { count: 'exact', head: true }).eq('income_category_id', id)

    if ((count || 0) > 0) return NextResponse.json({ error: 'Cannot delete category with existing transactions' }, { status: 400 })

    const { error } = await supabaseAdmin.from('income_categories').delete().eq('id', id)
    if (error) throw error
