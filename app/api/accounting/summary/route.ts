...
export async function GET(request: Request) {
  try {
    ...
    if (accountId) incomeByCatQuery = incomeByCatQuery.eq('account_id', accountId)
    if (dateFrom) incomeByCatQuery = incomeByCatQuery.gte('income_date', dateFrom)
    ...
    if (accountId) expenseByCatQuery = expenseByCatQuery.eq('account_id', accountId)
    if (dateFrom) expenseByCatQuery = expenseByCatQuery.gte('expense_date', dateFrom)
    if (dateTo) expenseByCatQuery = expenseByCatQuery.lte('expense_date', dateTo)
    const { data: expenseByCatData } = await expenseByCatQuery

    // Calculate totals
    const totalIncome = (incomeData || []).reduce((s: number, t: any) => s + Number(t.amount), 0)
    const totalExpense = (expenseData || []).reduce((s: number, t: any) => s + Number(t.amount), 0)
