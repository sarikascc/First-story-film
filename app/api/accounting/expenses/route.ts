...
export async function POST(request: Request) {
  try {
    ...
    if (!body.expense_date || !body.account_id || !body.expense_category_id || !body.amount) {
    ...
    const { data: category } = await supabaseAdmin.from('expense_categories').select('status').eq('id', body.expense_category_id).single()
    ...
    const { data, error } = await supabaseAdmin
      ...
      .select(`*, accounts(id, account_name), expense_categories(id, name), users(id, name)`)
      .single()
